...
const { currentTrack, currentArtist, trackTime, isLyrics } =
  storeToRefs(useSong);
...
<template>
  ...
  >
    ...
    <div class="w-1/2 max-w-[400px] mx-auto relative flex items-center">
      <div
        class="absolute z-20 font-bold rounded-xl text-white text-[20px] py-1.5 px-3 m-1"
      >
        {{ currentArtist.name }}
      </div>
      <div
        class="absolute right-0 z-20 font-bold rounded-xl text-white text-[20px] py-1.5 px-3 m-1"
      >
        {{ currentTrack.name }}
      </div>
      <div class="relative">
        ...
        <div
          class="absolute z-10 blur-2xl rounded-3xl top-0 w-full h-full bg-black bg-opacity-10"
        />
      </div>
    </div>
    <div
      id="LyricsDiv"
      class="w-1/2 max-w-[600px] mx-auto relative h-[calc(100%-1px)] overflow-auto scrollbar-hide"
    >
      <div class="my-[90%]"></div>
      <div
